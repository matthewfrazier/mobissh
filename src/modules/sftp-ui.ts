...
import {
  sftpState, openSftp, navigate, navigateUp, refreshCurrentDir,
  downloadFile, downloadToClipboard, downloadDir, batchDownload,
  uploadFiles, uploadDirectory, uploadFromClipboard, setTransferPendingRename,
  enterSelectMode, exitSelectMode, toggleSelect, selectAll, selectNone,
  rm, rmRecursive, mkdir, rename,
  formatFileSize, formatMtime,
} from './sftp.js';
import type { SftpEntry, SftpTransfer } from './types.js';
...
export function initSftpUI({ toast }: { toast: (msg: string) => void }): void {
  _toast = toast;
  _attachTabListener();
}

/** Called from sftp.ts whenever state changes — re-renders the panel. */
export function renderFilesPanel(): void {
  _renderToolbar();
  _renderFileList();
  _renderTransferList();
  _renderActionBar();
...
function _attachTabListener(): void {
  document.querySelector<HTMLElement>('[data-panel="files"]')?.addEventListener('click', () => {
...
function _renderToolbar(): void {
  ...
  toolbar.querySelectorAll<HTMLElement>('.sftp-crumb').forEach((crumb) => {
    crumb.addEventListener('click', () => {
      const p = crumb.dataset.path;
      if (p && p !== sftpState.currentPath) navigate(p);
    });
  });
}

function _buildBreadcrumbs(fullPath: string): { label: string; path: string }[] {
...
function _renderEntry(entry: SftpEntry): string {
...
function _renderTransferList(): void {
  ...
  area.querySelectorAll<HTMLElement>('.sftp-transfer-row').forEach((row) => {
...
function _renderTransferRow(t: SftpTransfer): string {
...
function _startInlineRename(row: HTMLElement, transferId: string): void {
  const nameEl = row.querySelector<HTMLElement>('.sftp-transfer-name');
  if (!nameEl) return;
  const currentName = nameEl.textContent || '';
  nameEl.innerHTML = `<input class="sftp-rename-input" type="text" value="${escHtml(currentName)}" />`;
  const input = nameEl.querySelector<HTMLInputElement>('.sftp-rename-input');
  if (!input) return;
  input.focus();
  input.select();
  const commit = (): void => {
  ...
  input.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Escape') { renderFilesPanel(); }
  });
}
...
function _showUploadSheet(): void {
...
function _openFilePicker(dirMode: boolean): void {
...
function _promptNewFolder(): void {
  ...
  const input = overlay.querySelector<HTMLInputElement>('.sftp-prompt-input');
  input?.focus();
...
function _confirmDelete(paths: string[], hasDir: boolean): void {
  const overlay = document.createElement('div');
  overlay.className = 'sftp-prompt-overlay';
  ...
  overlay.querySelector('.sftp-prompt-ok')?.addEventListener('click', () => {
    ...
    if (sftpState.selectMode) exitSelectMode();
