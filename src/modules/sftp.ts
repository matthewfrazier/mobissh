...
import type { SftpEntry, SftpDirCache, SftpTransfer, SftpMessage, SftpDeps } from './types.js';
...
export interface SftpState {
  ...
  homedir: string;
  currentPath: string;
  /** paths currently being loaded (pending readdir) */
  loadingPaths: Set<string>;
  /** sorted directory entries for currentPath */
  entries: SftpEntry[];
  dirCache: Map<string, SftpDirCache>;
  transfers: Map<string, SftpTransfer>;
  selectMode: boolean;
  selectedPaths: Set<string>;
}

export const sftpState: SftpState = {
  ready: false,
  homedir: '/',
  currentPath: '/',
...
function _sortEntries(entries: SftpEntry[]): SftpEntry[] {
...
export function initSftp({ toast, onStateChange }: SftpDeps): void {
  _toast = toast;
...
function _handleSftpMessage(rawMsg: unknown): void {
  const msg = rawMsg as SftpMessage;
  switch (msg.type) {
    case 'sftp_ready':
    ...
    case 'sftp_upload_progress': {
      const t = sftpState.transfers.get(msg.transferId);
      if (t) { t.receivedSize = msg.received; _notify(); }
      break;
    }
...
function _fetchDir(dirPath: string, background = false, force = false): void {
  if (sftpState.loadingPaths.has(dirPath)) return;
...
async function _saveToClipboard(blob: Blob, mimeType: string, filename: string, transferId: string): Promise<void> {
  try {
...
function _isTextFile(filename: string): boolean {
...
function _isImageFile(filename: string): boolean {
...
export function navigate(dirPath: string): void {
...
export function refreshCurrentDir(): void {
  sftpState.dirCache.delete(sftpState.currentPath);
  _fetchDir(sftpState.currentPath, false, true);
}

export function downloadFile(entry: SftpEntry, destDir: string): void {
...
export function downloadToClipboard(entry: SftpEntry, destDir: string): void {
  const remotePath = _joinPath(destDir, entry.name);
  ...
  if (!(navigator.clipboard as Clipboard | null | undefined)) {
...
export function downloadDir(dirPath: string): void {
  const id = _genId();
...
export function batchDownload(paths: string[]): void {
  const id = _genId();
  sftpState.transfers.set(id, {
...
export async function uploadFiles(files: FileList | File[], destDir: string): Promise<void> {
  for (const file of files) {
    await _uploadOneFile(file, destDir);
  }
}

export async function uploadDirectory(files: FileList, destDir: string): Promise<void> {
...
async function _uploadOneFile(file: File, destDir: string, remotePath?: string): Promise<void> {
...
export async function uploadFromClipboard(destDir: string): Promise<void> {
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
...
export function setTransferPendingRename(transferId: string, newName: string): void {
  const t = sftpState.transfers.get(transferId);
  if (!t) return;
...
export function enterSelectMode(firstPath?: string): void {
  sftpState.selectMode = true;
  sftpState.selectedPaths.clear();
  if (firstPath) sftpState.selectedPaths.add(firstPath);
  _notify();
}
...
export function toggleSelect(entryPath: string): void {
...
export function mkdir(parentDir: string, name: string): void {
...
export function rm(remotePath: string): void {
  sendWsMessage({ type: 'sftp_rm', path: remotePath });
  setTimeout(() => { refreshCurrentDir(); }, 300);
}

export function rmRecursive(remotePath: string): void {
  const id = _genId();
  sendWsMessage({ type: 'sftp_rm_recursive', path: remotePath, transferId: id });
}

export function rename(oldPath: string, newName: string): void {
...
export function formatMtime(unixTs: number): string {
  if (!unixTs) return '';
