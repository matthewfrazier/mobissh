...
import { vaultExists, isVaultLocked, lockVault, createVault, unlockWithPassword, changePassword, prfAvailable, enrollBiometric, disableBiometric, resetVault, tryUnlockVault, hasLegacyVault, } from './vault.js';
let _toast = (_msg) => { };
export function initVaultUI({ toast }) {
    _toast = toast;
    _initSetupModal();
    _initUnlockBar();
    _initChangePwModal();
...
export function showVaultSetup() {
    ...
    if (prfAvailable()) {
        ...
        document.getElementById('vaultEnableBio').checked = true;
    }
    else {
...
function _initSetupModal() {
    ...
    createBtn.addEventListener('click', () => {
        ...
        errorEl.classList.add('hidden');
        const enrollBio = prfAvailable() && document.getElementById('vaultEnableBio').checked;
 ...
 */
export function showUnlockBar() {
    const bar = document.getElementById('vaultUnlockBar');
    const pwInput = document.getElementById('vaultUnlockPw');
...
function _initChangePwModal() {
    const overlay = document.getElementById('vaultChangePwOverlay');
    const oldPwInput = document.getElementById('vaultOldPw');
    const newPwInput = document.getElementById('vaultChangePwNew');
    const confirmInput = document.getElementById('vaultChangePwConfirm');
...
function _initVaultSettings() {
    ...
    document.getElementById('vaultResetBtn').addEventListener('click', () => {
        if (!confirm('This will delete all saved credentials. You will need to re-enter them.'))
            return;
        resetVault();
        _toast('Vault reset');
        updateVaultSettingsUI();
...
export function updateVaultSettingsUI() {
    ...
    if (hasLegacyVault() && !vaultExists()) {
        if (statusEl)
            statusEl.textContent = 'Legacy vault (needs migration)';
    ...
    if (locked) {
        ...
        if (statusEl)
            statusEl.textContent = hasBio ? 'Unlocked (password + fingerprint)' : 'Unlocked (password only)';
 ...
 */
export async function promptVaultSetupOnStartup() {
    if (vaultExists())
        return;
    if (hasLegacyVault())
        return;
 ...
 */
export async function ensureVaultKeyWithUI() {
    // Already unlocked
    if (appState.vaultKey)
        return true;
...
//# sourceMappingURL=vault-ui.js.map